        ...
        if (is_array($value)) {
            ...
            if (!preg_match($regex, $value, $match)) {
                throw new sfValidatorError($this, 'bad_format', array(
                    'value' => $value,
                    'date_format' => $this->getOption('date_format_error') ? $this->getOption('date_format_error') : $this->getOption('date_format'))
                );
            }
            ...
            # modified $value to be assigned into a new variable $tainted_value
            # previously the entire method overwrote $value - buggy code
            $tainted_value = $value;
            
            list($d, $m, $y) = $i18n->getDateForCulture($tainted_value, $context->getUser()->getCulture());
            $value = "$y-$m-$d";
            
            if ($this->getOption('with_time')) {
                list($h, $m) = $i18n->getTimeForCulture($tainted_value, $context->getUser()->getCulture());
                
                # symfony's i18n getTimeForCulture method is broken it adds 12 hours for no reason
                # this only applies when he hour is 12am or 12pm
                if ($h % 12 == 0) {
                   $h = $h - 12;
                }
